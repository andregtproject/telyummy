    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('menu-items.update', $menuItem) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <x-menu-image-upload
                        ...
                        helpText="PNG, JPG, JPEG maksimal 2MB. Kosongkan jika tidak ingin mengubah foto."
                    />
                    ...
                    <x-menu-category-autocomplete
                        ...
                        :initialValue="old('category', $menuItem->category)"
                    />
            ...
            <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-red-600">Hapus Menu</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        Setelah menu dihapus, semua data terkait akan dihapus secara permanen.
                    </p>
                    <form action="{{ route('menu-items.destroy', $menuItem) }}" method="POST" class="mt-4"
                          onsubmit="return confirm('Yakin ingin menghapus menu {{ $menuItem->name }}? Aksi ini tidak dapat dibatalkan.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-red-100 border border-transparent rounded-lg font-semibold text-xs text-red-700 uppercase tracking-widest hover:bg-red-200 focus:bg-red-200 active:bg-red-300 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
